...
class TiledLevel extends TiledMap
{
	...
	public var foregroundTilesLayer:FlxGroup;
	public var objectsLayer:FlxGroup;
	public var backgroundTilesLayer:FlxGroup;
	private var collidableTilesLayers:Array<FlxTilemap>;

	// Sprites of images layers
	public var backgroundImagesLayer:FlxGroup;

	public function new(tiledLevel:Dynamic, state: GameState)
	{
		super(tiledLevel);

		backgroundImagesLayer = new FlxGroup();
		foregroundTilesLayer = new FlxGroup();
		objectsLayer = new FlxGroup();
		backgroundTilesLayer = new FlxGroup();
		...
		for (layer in layers)
		{
			...
			{
				backgroundTilesLayer.add(tilemap);
			}
			else
			{
				if (collidableTilesLayers == null)
					collidableTilesLayers = new Array<FlxTilemap>();

				foregroundTilesLayer.add(tilemap);
				collidableTilesLayers.push(tilemap);
			}
		}
	}
	...
	{
		var tilesImageCollection:TiledTileSet = this.getTileSet("fondo");
		...
		backgroundTilesLayer.add(decoSprite);
	}
	...
	private function loadObject(state: GameState, o:TiledObject, g:TiledObjectLayer, group:FlxGroup)
	{
		...
		switch (o.type.toLowerCase())
		{
			case "player_start":
				...
				// state.player = player;
				// group.add(player);
				state.PLAYER_INIT_POS = new FlxPoint(x, y);
			...
			case "pickup":
				...
				var pickup = new Pickup(x, y);
				state.pickupAmount++;
				state.pickups.add(pickup);
				group.add(pickup);

			case "exit":
				// Create the level exit door
				var door = new Door(x, y);
				state.door = door;
				group.add(door);

			case "button":
				var id = Std.parseInt(o.properties.id);
				var button = new Button(x, y, id);
				state.buttons.add(button);
				group.add(button);

			case "platform":
				var id = Std.parseInt(o.properties.id);
				var plat = new Platform(x, y, id, o.properties.direction);
				state.platforms.add(plat);
				group.add(plat);

			case "spikes":
				var spikes = new Spikes(x, y);
	...
	public function loadImages()
	{
		for (layer in layers)
		{
			...
			backgroundImagesLayer.add(sprite);
		}
	}
	...
	public function collideWithLevel(obj:FlxObject, ?notifyCallback:FlxObject->FlxObject->Void, ?processCallback:FlxObject->FlxObject->Bool):Bool
	{
		...
		for (map in collidableTilesLayers)
		{
